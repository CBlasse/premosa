...
static inline int allocate_ctour_vert(Ctour *ctour, int vsize, char *routine)
{ _Ctour *object = (_Ctour *) (((char *) ctour) - Ctour_Offset);
  if (object->vsize < vsize)
    { void *x = Guarded_Realloc(ctour->vert,(size_t) vsize,routine);
      if (x == NULL) return (1);
      ctour->vert = x;
      object->vsize = vsize;
    }
...
static inline int sizeof_ctour_vert(Ctour *ctour)
{ _Ctour *object = (_Ctour *) (((char *) ctour) - Ctour_Offset);
  return (object->vsize);
}

static inline int allocate_ctour_strt(Ctour *ctour, int ssize, char *routine)
{ _Ctour *object = (_Ctour *) (((char *) ctour) - Ctour_Offset);
  if (object->ssize < ssize)
    { void *x = Guarded_Realloc(ctour->strt,(size_t) ssize,routine);
      if (x == NULL) return (1);
      ctour->strt = x;
      object->ssize = ssize;
    }
...
static inline int sizeof_ctour_strt(Ctour *ctour)
{ _Ctour *object = (_Ctour *) (((char *) ctour) - Ctour_Offset);
  return (object->ssize);
}
...
static inline Ctour *new_ctour(int tsize, int vsize, int ssize, char *routine)
{ _Ctour *object;
  Ctour  *ctour;
  ...
  if (allocate_ctour_vert(ctour,vsize,routine))
    { kill_ctour(ctour);
      return (NULL);
    }
  if (allocate_ctour_strt(ctour,ssize,routine))
    { kill_ctour(ctour);
      return (NULL);
    }
  return (ctour);
...
{ Ctour *copy = new_ctour(ctour_tsize(ctour),ctour_vsize(ctour),ctour_ssize(ctour),"Copy_Contour");
  void *_tour = copy->tour;
  void *_vert = copy->vert;
  void *_strt = copy->strt;
  *copy = *ctour;
  copy->tour = _tour;
  if (ctour->tour != NULL)
  ...
  copy->vert = _vert;
  if (ctour->vert != NULL)
    memcpy(copy->vert,ctour->vert,(size_t) ctour_vsize(ctour));
  copy->strt = _strt;
  if (ctour->strt != NULL)
    memcpy(copy->strt,ctour->strt,(size_t) ctour_ssize(ctour));
  return (copy);
}
...
{ _Ctour *object  = (_Ctour *) (((char *) ctour) - Ctour_Offset);
  ...
  if (object->vsize > ctour_vsize(ctour))
    { int ns = ctour_vsize(ctour);
      if (ns != 0)
        { void *x = Guarded_Realloc(ctour->vert,(size_t) ns,"Pack_Ctour");
          if (x == NULL) return (1);
          ctour->vert = x;
        }
      else
  ...
  if (object->ssize > ctour_ssize(ctour))
    { int ns = ctour_ssize(ctour);
      if (ns != 0)
        { void *x = Guarded_Realloc(ctour->strt,(size_t) ns,"Pack_Ctour");
          if (x == NULL) return (1);
          ctour->strt = x;
        }
      else
...
{ char name[7];
  ...
  if (strncmp(name,"Contour",7) != 0)
    return (NULL);
  obj = new_ctour(0,0,0,"Read_Contour");
  ...
  obj->vert = read.vert;
  if (ctour_vsize(obj) != 0)
    { if (allocate_ctour_vert(obj,ctour_vsize(obj),"Read_Contour")) goto error;
      if (fread(obj->vert,(size_t) ctour_vsize(obj),1,input) == 0) goto error;
    }
  obj->strt = read.strt;
  if (ctour_ssize(obj) != 0)
    { if (allocate_ctour_strt(obj,ctour_ssize(obj),"Read_Contour")) goto error;
      if (fread(obj->strt,(size_t) ctour_ssize(obj),1,input) == 0) goto error;
    }
  return (obj);
...
{ fwrite("Contour",7,1,output);
  ...
  if (ctour_vsize(ctour) != 0)
    fwrite(ctour->vert,(size_t) ctour_vsize(ctour),1,output);
  if (ctour_ssize(ctour) != 0)
    fwrite(ctour->strt,(size_t) ctour_ssize(ctour),1,output);
}
